...
export function GuidedTour({ isActive, onClose, onSpeak }: GuidedTourProps) {
  ...
  return (
    <>
      ...
      >
        <div className="bg-background border-2 border-muted-foreground/30 shadow-2xl">
          <div className="bg-foreground text-background px-6 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-sans text-xs text-background/60 uppercase tracking-wider">
                    Guided Tour
                  </span>
          ...
          <div className="px-6 py-4 bg-secondary/50 border-t border-muted-foreground/20 flex items-center justify-between">
              ...
              {tourSteps.map((_, index) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full transition-colors ${
                    index === currentStep ? 'bg-primary' : 'bg-muted-foreground/30 hover:bg-muted-foreground/50'
                  }`}
            ...
            <div className="flex items-center gap-3">
              {currentStep > 0 && (
                <button
                  onClick={prevStep}
                  className="px-4 py-2 font-sans text-sm text-foreground hover:text-primary transition-colors"
                >
                  Previous
                </button>
              )}
              <button
                onClick={nextStep}
                className="px-5 py-2 bg-foreground text-background font-sans text-sm hover:bg-primary transition-colors"
              >
